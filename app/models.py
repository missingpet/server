...
class UserManager(BaseUserManager):
    ...
    def create_user(self, email, username, photo=None, password=None):
        if email is None:
        ...
        user = self.model(email=self.normalize_email(email), username=username, photo=photo)
        user.set_password(password)
        user.save()
        return user

    def create_superuser(self, email, username, photo=None, password=None):
        if password is None:
            raise TypeError('Пароль не может иметь значение None.')
        user = self.create_user(username=username, email=email, password=password, photo=photo)
...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    photo = models.ImageField(upload_to='users', blank=True, null=True, verbose_name='Фотография пользователя')
    ...
    def save(self, *args, **kwargs):
        try:
            ...
            if this_user.photo != self.photo:
                this_user.photo.delete(save=False)
        except:
            pass
...
class Announcement(models.Model):
    ...
    breed = models.CharField(max_length=50, blank=True, null=True, verbose_name='Порода')
    ...
    description = models.CharField(max_length=500, null=True, blank=True, verbose_name='Комментарий')
    contact_phone_number = models.CharField(max_length=12, null=True, blank=True, verbose_name='Контактный телефон')
    created_at = models.DateTimeField(auto_now_add=True, verbose_name='Создано')
    updated_at = models.DateTimeField(auto_now=True, verbose_name='Изменено')
    at_home = models.BooleanField(default=False, verbose_name='Уже дома')
    ...
    def __str__(self):
        return f'title: {self.title}; announcement_type: {self.announcement_type}; at_home: {self.at_home}; id: {self.id}'
